<?php 
add_action( 'wp_enqueue_scripts', 'ocean_blog_enqueue_styles' );
function ocean_blog_enqueue_styles() {
	wp_enqueue_style( 'ocean-blog-parent-style', get_template_directory_uri() . '/style.css' ); 
} 


function ocean_blog_google_fonts() {
	wp_enqueue_style( 'ocean-blog-google-fonts', '//fonts.googleapis.com/css?family=Inter:400,600', false ); 
}
add_action( 'wp_enqueue_scripts', 'ocean_blog_google_fonts' );
 ...
 */
function ocean_blog_customize_preview_js() {
	wp_enqueue_script( 'ocean-blog-customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'ocean_blog_customize_preview_js' );


function ocean_blog_customize_register( $wp_customize ) {
...
$wp_customize->add_setting( 'header_img_text', array(
	...
	$wp_customize->add_control( 'readmoretext', array(
		'label'    => __( "Read More Text", 'ocean-blog' ),
		'section'  => 'header_image',
		'type'     => 'text',
		'priority' => 1,
		) );
	...
	$wp_customize->add_control( 'readmorelink', array(
		'label'    => __( "Read More Link URL", 'ocean-blog' ),
		'section'  => 'header_image',
		'type'     => 'text',
		'priority' => 1,
		) );
	...
	$wp_customize->add_setting( 'ocean_blog_readmore_text_color', array(
		'default'           => '#fff',
		'sanitize_callback' => 'sanitize_hex_color',
		'transport'         => 'refresh',
	) );
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'ocean_blog_readmore_text_color', array(
		'label'       => __( 'Read More Button Text Color', 'ocean-blog' ),
		'section'     => 'colors',
		'priority'   => 1,
		'settings'    => 'ocean_blog_readmore_text_color',
	) ) );



	$wp_customize->add_setting( 'ocean_blog_readmore_bg_color', array(
		'default'           => '#0067e5',
		'sanitize_callback' => 'sanitize_hex_color',
		'transport'         => 'refresh',
	) );
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'ocean_blog_readmore_bg_color', array(
		'label'       => __( 'Read More Button Background Color', 'ocean-blog' ),
		'section'     => 'colors',
		'priority'   => 1,
		'settings'    => 'ocean_blog_readmore_bg_color',
	) ) );



	function ocean_blog_sanitize_checkbox( $input ) {
...
add_action( 'customize_register', 'ocean_blog_customize_register' );





if(! function_exists('ocean_blog_customizer_css_final_output' ) ):
	function ocean_blog_customizer_css_final_output(){
		?>
	...
	<?php }
	add_action( 'wp_head', 'ocean_blog_customizer_css_final_output', 99999 );
endif;